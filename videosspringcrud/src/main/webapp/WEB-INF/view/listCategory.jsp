					...
					<table class="table table-striped">
						...
						<c:if test="${how_many > 0}">
							<c:forEach items="${categories}" var="category">
								...
								<tr>
									<td>${category.name}</td>									
									<td><fmt:formatDate value="${category.updatedAt}" type="both"
											dateStyle="long" timeStyle="short" /></td>									
									<td><a href="${edit}" title="Actualizar ${category.name}"><button
												type="submit" class="btn btn-success">Modificar</button></a></td>
									<td><a href="${delete}" title="Borrar ${category.name}"><button
